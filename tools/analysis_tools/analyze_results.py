...
def bbox_map_eval(det_result, annotation, nproc=4):
    ...
    #print(bbox_det_result, annotation)
    mean_ap, ev_res, ap50_per_obj = eval_rbbox_map(bbox_det_result, [annotation], iou_thr=0.5, logger='silent', nproc=1)
    print(ap50_per_obj)
    """
    for thr in iou_thrs:
        p = workers.apply_async(eval_rbbox_map, (bbox_det_result, [annotation]), {
            'iou_thr': thr,
            'logger': 'silent',
            'nproc': 1
        })
        ...
        mean_ap, ev_res, aps_per_obj = eval_rbbox_map(bbox_det_result, [annotation], iou_thr=thr, logger='silent', nproc=4)
        #p.append(mean_ap*100)
        print(f'AP{int(thr*100)}: {mean_ap*100}')
    """
...
class ResultVisualizer:
    ...
    def evaluate_and_show(self,
        ...
        if isinstance(results[0], dict):
            ...
            #good_samples, bad_samples = self.detection_evaluate(
                #dataset, results, topk=topk)
            mAP = self.detection_evaluate(dataset, results, topk=topk)
    ...
    def detection_evaluate(self, dataset, results, topk=20, eval_fn=None):
        ...
        for i, (result, ) in enumerate(zip(results)):
            ...
            aps50 = eval_fn(result, data_info['ann_info'])
